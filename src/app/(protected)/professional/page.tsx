...
export default function ProfessionalsPage() {
    ...
    const fetchProfessionals = async () => {
        setLoading(true);
        try {
            const data = await adminService.getProfessionals(page, 10, search);
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    <Text variant="h3" className="text-gray-900">Profissionais</Text>
                    <Text variant="body" className="text-gray-500">Gerencie os marceneiros e parceiros da plataforma.</Text>
                </div>
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {loading ? (
                                ...
                                professionals.map((pro) => (
                                    <tr key={pro._id} className="hover:bg-gray-50/50 transition-colors">
                                        ...
                                        <td className="px-6 py-4 text-gray-600">
                                            <div className="flex flex-col">
                                                <span>{pro.email}</span>
                                                <span className="text-xs text-gray-400">{pro.phone || '-'}</span>
                                            </div>
                                        </td>
                                        <td className="px-6 py-4">
                                            <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${pro.isActive ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700'}`}>
